...
tag_list = ['measure','measure-attributes-time','measure-note','measure-note-pitch','measure-note-duration','measure-note-stem','measure-note-duration','measure-note-rest']
...
def extract_info(fileName):
    ...
    for measure in root.iter("measure"): 
        ...
        for clefLabel in measure.iter('clef'):
            ...
            clefs.append([measureNum, clefInfo])
        
        
        #key-fifths catch
        for keyLabel in measure.iter('fifths'):
            key_fifth.append([measureNum, 'key'+keyLabel.text])
        ...
        if (measureNum>=startMeasure) and (measureNum<=endMeasure or endMeasure ==-1):
            ...
            for note in measure.iter('note'):
                ...
                for pitch in note.iter('pitch'):
                    ...
                    if locationMap.__contains__(clefofthismea+'-'+pitch_midi):
                        everyline_label.append(pitch_time+ str(locationMap[clefofthismea+'-'+pitch_midi]))
    ...
    df.to_csv('labels/'+fileName+'.csv',index=False,header=True)
